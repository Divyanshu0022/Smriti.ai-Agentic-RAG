document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.audit-btn').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            try {
                // All audits (including professional) use the rich text RAG endpoint for display
                const res = await fetch('/api/audit', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ type: auditType, model: model })
                });
                const data = await res.json();
                document.getElementById(thinkingId)?.remove();
                
                if (res.ok) {
                    // For professional audit, render the Append to Sheet button
                    const isProf = auditType === 'professional';
                    appendMessage('ai', `**${auditType.toUpperCase()} AUDIT REPORT**\n\n${data.audit}`, null, null, null, null, false, isProf);
                } else {
    ...
    function appendMessage(role, text, chunks = null, scores = null, sources = null, responseTime = null, cached = false, profileData = null) {
        ...
        if (role === 'ai') {
            ...
            const isProfileAudit = !!profileData;
            contentHtml += `
                <div class="msg-actions">
                    <button class="msg-action-btn" onclick="takeAction('${contentId}', 'email')" title="Email this summary">
                        <i class="fa-solid fa-envelope"></i> Email Report
                    </button>
                    <button class="msg-action-btn" onclick="takeAction('${contentId}', 'ats_brief')" title="Send ATS-style professional brief">
                        <i class="fa-solid fa-file-lines"></i> Email Professional Brief
                    </button>
                    ${isProfileAudit ? `<button class="msg-action-btn" onclick="appendSheetFromAudit()" title="Append this profile to Google Sheet">
                        <i class="fa-solid fa-file-export"></i> Append to Sheet
                    </button>` : ''}
                </div>
...
// ─── Global: Last profile audit result (set after Profile Expert audit) ───
let _lastProfileAuditData = null;
...
async function takeAction(elementId, type) {
    ...
    let subject = 'Smriti.ai — Document Report';
    let body    = content;

    if (type === 'ats_brief') {
        subject = 'Candidate Professional Brief (ATS-Ready)';
        body = `Dear Hiring Manager,\n\nPlease find below an ATS-formatted professional summary generated from the candidate's resume:\n\n${content}\n\nThis brief was auto-generated by Smriti.ai agentic pipeline.`;
    }
    ...
    try {
        const res = await fetch('/api/action', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'email', content: body, recipient, subject })
        });
        const data = await res.json();
        alert(data.message);
    } catch (e) {
...
async function appendSheetFromAudit() {
    ...
    try {
        ...
        const data = await res.json();
        if (data.success) {
            alert('✅ Profile successfully appended to Google Sheet!');
        } else {
...
// Legacy alias used by below-chatbox button
function appendSheet() { appendSheetFromAudit(); }
